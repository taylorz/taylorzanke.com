...
// New hooks for archive pages
export function useArchiveYears() {
  const { data, error, isLoading } = useSWR(workQueries.allYears, fetcher);
...
export function useWorksByYear(year) {
  const { data, error, isLoading } = useSWR(
    year ? [workQueries.byYear, { year: Number(year) }] : null,
    ([query, params]) => fetcher(query, params)
  );
